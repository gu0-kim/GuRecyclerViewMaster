...
public class GuRecyclerView extends RecyclerView {
    ...
    private GuRefreshHeader mRefreshHeader;
    ...
    public GuRecyclerView(Context context) {
        this(context, null);
    }

    public GuRecyclerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public GuRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context) {
        if (pullRefreshEnabled) {
            GuRefreshHeader refreshHeader = new GuRefreshHeader(context);
    ...
    public void addHeaderView(View view) {
        if (pullRefreshEnabled && !(mHeaderViews.get(0) instanceof GuRefreshHeader)) {
            GuRefreshHeader refreshHeader = new GuRefreshHeader(getContext());
            mHeaderViews.put(0, refreshHeader);
            mRefreshHeader = refreshHeader;
        }
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && !isLoadingData && loadingMoreEnabled) {
            ...
            if (layoutManager.getChildCount() > 0 && lastVisibleItemPosition >= layoutManager.getItemCount() - 1 && layoutManager.getItemCount() > layoutManager.getChildCount() && !isnomore && mRefreshHeader.getState() < GuRefreshHeader.STATE_REFRESHING) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isOnTop() && pullRefreshEnabled) {
                    ...
                    if (mRefreshHeader.getVisiableHeight() > 0 && mRefreshHeader.getState() < GuRefreshHeader.STATE_REFRESHING) {
                        return false;
                    }
                }
                break;
    ...
    public boolean isOnTop() {
        return !(mHeaderViews == null || mHeaderViews.size() == 0) && mHeaderViews.get(0).getParent() != null;
    }
